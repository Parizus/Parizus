function sn = CalculateSN(ek,em,en,Disp_,ith,cf,Mode,Modenum,nodnum)
	%system stiffness matrix
	k(1:2*nodnum,1:2*nodnum)=0;
	%system mass matrix
	m(1:2*nodnum,1:2*nodnum)=0;
	index(1:8)=0; % vector sontaining system dofs of nodes in each element.
	loopi=ith;
		for zi=1:4
			index((zi-1)*2+1)=Disp_(en(loopi,zi),1);
			index((zi-1)*2+2)=Disp_(en(loopi,zi),2);
		end
		for i=1:8
			for j=1:8
